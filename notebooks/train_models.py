...
def train_dengue_model(city, end_date_train='2022-11-01', end_date='2023-11-01', ini_date=None):
    ...
    m_msle_all, h_msle_all, m_train_all, m_val_all = make_pred(model, city, doenca='dengue', epochs=EPOCHS,
                                                               end_train_date=end_date_train,
                                                               ini_date=ini_date,
                                                               ratio=None, end_date=end_date,
                                                               predict_n=PREDICT_N, look_back=LOOK_BACK, label='msle',
                                                               filename=FILENAME_DATA, verbose=0)
...
def train_chik_model(city, end_date_train='2022-11-01', end_date='2023-11-01', ini_date=None):
    ...
    m_msle_all, h_msle_all, m_train_all, m_val_all = make_pred(model, city, doenca='chik', epochs=EPOCHS,
                                                               end_train_date=end_date_train,
                                                               ini_date=ini_date,
                                                               ratio=None, end_date=end_date,
                                                               predict_n=PREDICT_N, look_back=LOOK_BACK, label='msle',
...
def transf_and_pred_chik(city, ini_date, end_date_train, end_date):
    ...
    m_msle, hist, m_t, m_val = transf_chik_pred(model, city, ini_date=ini_date, end_train_date=end_date_train,
                                                ...
                                                label=f'transf_msle', ratio=None, filename_data=FILENAME_DATA,
                                                verbose=1)
...
def train_pgbm_dengue(city, state, ini_date, end_date_train, end_date):
    ...
    preds, preds25, preds975, X_train, targets = pgbm_pred(city, state, PREDICT_N, LOOK_BACK, doenca='dengue',
...
def apply_dengue_pgbm_on_chik(city, state, ini_date, end_date):
    FILENAME_DATA = f'../data/chik_{city}_cluster.csv'
    preds, preds25, preds975, X_data, targets = cross_dengue_chik_prediction(city, state, predict_n=PREDICT_N,
...
def train_pgbm_chik(city, state, ini_date, end_date_train, end_date):
    ...
    preds, preds25, preds975, X_train, targets = pgbm_pred(city, state, PREDICT_N, LOOK_BACK, doenca='chik',
                                                           ini_date=ini_date, end_train_date=end_date_train,
                                                           end_date=end_date, filename=FILENAME_DATA)
